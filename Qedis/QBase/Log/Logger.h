...
#define BASE_FILENAME(f) (strrchr(f,'/')?strrchr(f,'/')+1:f)

#define  LOG_DBG(x)      (LogHelper(logDEBUG))=(x ? x : LogManager::Instance().NullLog())->SetCurLevel(logDEBUG) << BASE_FILENAME(__FILE__) << ":" << __LINE__ << " "
#define  LOG_INF(x)      (LogHelper(logINFO))=(x ? x : LogManager::Instance().NullLog())->SetCurLevel(logINFO) << BASE_FILENAME(__FILE__) << ":" << __LINE__ << " "
#define  LOG_WRN(x)      (LogHelper(logWARN))=(x ? x : LogManager::Instance().NullLog())->SetCurLevel(logWARN) << BASE_FILENAME(__FILE__) << ":" << __LINE__ << " "
#define  LOG_ERR(x)      (LogHelper(logERROR))=(x ? x : LogManager::Instance().NullLog())->SetCurLevel(logERROR) << BASE_FILENAME(__FILE__) << ":" << __LINE__ << " "
#define  LOG_USR(x)      (LogHelper(logUSR))=(x ? x : LogManager::Instance().NullLog())->SetCurLevel(logUSR) << BASE_FILENAME(__FILE__) << ":" << __LINE__ << " "
