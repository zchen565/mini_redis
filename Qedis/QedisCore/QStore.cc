...
QObject::QObject(QType t) : type(t)
{
    ...
    if (type != QType_invalid)
        USR << "Create qobject with type " << (int)type;
#endif
}
...
#ifdef LAB3
    USR << "Delete qobject with type " << type;
#endif
}
...
QError  QStore::_GetValueByType(const QString& key, QObject*& value, QType type, bool touch)
{
    ...
    if (cobj)
    {
        ...
        {
#ifdef LAB3
            ERR << "GetValue type mismatch: want " << type << ", but type of " << key << " is " << QType(cobj->type);
#endif
            return QError_type;
        }
        else
        {
#ifdef LAB3
            INF << "GetValue success: type " << type << ", key " << key;
#endif
...
{
#ifdef LAB3
    USR << "SetValue with key " << key << ", value type " << value.type;
#endif
    auto db = &store_[dbno_];
